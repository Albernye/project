...
from algorithms.fingerprint import set_origin, ll_to_local, local_to_ll
...
def run_combined_simple(merged_csv_path: str, qr_json_path: str):
    ...
    if num_steps == 0:
        ...
        for qr_reset in qr_resets:
            ...
            print(f"QR reset {qr_reset['room']} at sample {closest_idx}: "f"offset=({offset[0]:.2f}, {offset[1]:.2f})")

        # 8) Convert trajectories to lon/lat
        set_origin(origin_lat, origin_lon)

        trajectory_ll = np.array([local_to_ll(x, y) for x, y in trajectory])
        fused_trajectory_ll = np.array([local_to_ll(x, y) for x, y in fused_trajectory])

        print(f"Final PDR position (lon/lat): ({trajectory_ll[-1,0]:.6f}, {trajectory_ll[-1,1]:.6f})")
        print(f"Final fused position (lon/lat): ({fused_trajectory_ll[-1,0]:.6f}, {fused_trajectory_ll[-1,1]:.6f})")

        # 9) Plot results in local coordinates (meters)
        ...
        plt.title('PDR + QR Fusion Trajectory (meters)')
        plt.xlabel('X (m)')
        plt.ylabel('Y (m)')
        plt.legend()
        ...
        plt.subplot(1, 3, 2)
        plt.plot(t[:5000], magnitude[:5000], 'b-', alpha=0.7, label='|Acceleration|')
